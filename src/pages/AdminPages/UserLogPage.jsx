...
const UserLogPage = () => {
  ...
  /**
   * Delete a log entry by index from filteredLogs
   * 
   * @param {number} logIndex - Index of the log in filteredLogs to delete (filteredLogs does not have an id)
   */
  const handleDelete = (logIndex) => {
    if (deleteConfirm !== logIndex) {
      setDeleteConfirm(logIndex);
      return;
    }
    const logToDelete = filteredLogs[logIndex];
    if (!logToDelete) return;
    const updatedLogs = logs.filter(
      (log) => JSON.stringify(log) !== JSON.stringify(logToDelete)
    );
    const updatedFiltered = filteredLogs.filter((_, idx) => idx !== logIndex);
    setLogs(updatedLogs);
    setFilteredLogs(updatedFiltered);
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 p-6">
        {/* Loading state */}
        {loading ? (
          <div className="p-6 h-full flex justify-center items-center" aria-live="polite" role="status">
